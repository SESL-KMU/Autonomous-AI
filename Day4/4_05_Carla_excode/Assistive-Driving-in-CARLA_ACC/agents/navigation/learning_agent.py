...
class LearningAgent(Agent):
	...
	if self._state == AgentState.BLOCKED_BY_VEHICLE:
	'''
	...
	SS=self._state
        ...
        if self._state == AgentState.NAVIGATING:
		self.desired_speed = 0
		control = self._local_planner.run_step(debug=debug,target_speed=30)
	else:
		# ACC
		front_dis = self._front_r[1][0]
		front_vel = self._front_r[2][0]
		...
		u = (ego_speed*-2979)+(3061*front_dis)
		u += (1279*(-headway_input+front_dis))*(0.001) \
			+ (204*(-headway_input+front_dis))*(0.001)*(0.001)
		mg=u*0.001
		self.desired_speed+=(mg+(-0.0137*ego_speed))*0.001
		control = self._local_planner.run_step(debug=debug, target_speed=self.desired_speed*3.6)
		
	
	print("%s" %SS)
	print("desired speed : %s" %self.desired_speed)
	print("get speed : %s" %self._get_speed())
	'''
	    desired_speed = front_vel - (front_vel-ego_speed)/front_dis 2953 3061 1279 204
	    if ego_speed > 1:
		desired_speed += 2*(front_vel-ego_speed)*(0.0625) \
		+1.5*(front_vel-ego_speed)*(0.0625)*(0.0625)
	    control = self._local_planner.run_step(debug=debug, target_speed=desired_speed*3.6)
	'''
	'''desired_speed = -ego_speed*2953+3061*front_dis
	    if ego_speed > 1:
		desired_speed += 1279*(-20 + front_dis)*(0.0625) \
		+204*(-20 + front_dis)*(0.0625)*(0.0625)
	    control = self._local_planner.run_step(debug=debug, target_speed=desired_speed*3.6)'''  
