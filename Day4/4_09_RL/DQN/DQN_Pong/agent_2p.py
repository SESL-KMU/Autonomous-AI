...
class DeepLearningAgent:
    ...
    def load_progress(self, filename=None):
        """
        Loads the progress from the agent deep network
        """
        file_loaded = self.perception.attempt_restore(self.networks_directory, filename)
        if file_loaded:
    ...
    def act_and_perceive(self, action_selected, percept_stack, p2=[1,0,0]):
        """
        ...
        while True:
            ...
            if not self.train and self.environment.game_over():
                ...
                log.info(('agent won ' if won else 'cpu won ') +
                         ...
                         ' to ' + str(agent_games))
                print(('agent won ' if won else 'player won ') +
                         '... final score ' + str(cpu_games) +
                         ' to ' + str(agent_games))
                while True:
                    for event in pygame.event.get():
...
def main(args):
    ...
    raw_percept, _ = agent.environment.frame_step(noop, noop)
    ...
    agent.load_progress('/pretrained/network-5600000')
