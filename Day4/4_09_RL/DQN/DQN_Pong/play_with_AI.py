...
class DeepLearningAgent:
    ...
    def __init__(self, input_width, input_height, nactions, nimages, reset=False, train=False, players = 1, step = 0):
        ...
        self.step = 0
        self.perception = percept.MultilayerConvolutionalNetwork(None,
    ...
    def save_progress(self, stack):
        ...
        self.perception.save_network(self.networks_directory, self.step+self.model_step)

    def load_progress(self, model_name = None):
        """
        Loads the progress from the agent deep network
        """
        file_loaded = self.perception.attempt_restore(self.networks_directory, model_name)
        if file_loaded:
...
def main(args):
    ...
    model_path = 'pretrained/network-5499999'
    agent.load_progress(model_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help='clears the folders where the state of the agent is saves. '
             'NOTE: use this to retrain the agent from scratch',
        dest='players', type=int, default=2)
