...
class DeepLearningAgent:
    ...
    def __init__(self, input_width, input_height, nactions, nimages, reset=False, train=False, players = 1, step = 0):
        """
        ...
        with self.g1.as_default():
            self.perception1 = percept.MultilayerConvolutionalNetwork(self.g1, input_width, input_height, nimages, nactions)
        with self.g2.as_default():
            self.perception2 = percept.MultilayerConvolutionalNetwork(self.g2, input_width, input_height, nimages, nactions)
    ...
    def save_progress(self, stack):
        ...
        for i in range(stack.shape[2]):
            ...
            self.perception1.save_percepts(current_percept_path, stack[:,:,i])
        ...
        with open(self.logs_directory + '/readout.txt', 'w+') as a_file:
            with open(self.logs_directory + '/hidden.txt', 'w+') as h_file:
                self.perception1.save_variables(a_file, h_file, stack)

        log.info('saving current network')
        self.perception1.save_network(self.networks_directory, self.step+self.model_step+1)

    def load_progress(self, model1 = None, model2= None):
        """
        Loads the progress from the agent deep network
        """
        file_loaded1 = self.perception1.attempt_restore(self.networks_directory, model1)
        file_loaded2 = self.perception2.attempt_restore(self.networks_directory, model2)
        if file_loaded1 and file_loaded2:
            log.info('loaded successfully => ' + str(file_loaded1)+ str(file_loaded2))
        else:
            log.info("didn't find any saved network")

    def select_action(self, model,  x_t = None):
        """
        ...
        do_action = model.select_best_action(x_t)
    ...
    def learn_maybe(self):
        ...
        value_batch = []
        readout_batch = self.perception1.readout_act(state_prime_batch)
        ...
        self.perception1.train(value_batch, action_batch, state_batch)
        log.debug('end of training')

    def act_and_perceive(self, action_selected, percept_stack, p2=None):
        """
        ...
        new_percept, reward = self.environment.frame_step(action_selected, p2)
        log.debug('got reward of ' + str(reward))

        new_percept = self.perception1.preprocess_percepts(new_percept)
    ...
    def exist(self, percept_stack):
        ...
        while True:
            ...
            if self.players is 2:
                ...
                action_selected2 = [a0, a1, a2]
            start = time.time()
            action_selected = self.select_action(self.perception1, percept_stack)
            if self.players == 3:
                percept_stack2 = cv2.flip(percept_stack, 0)
                M = np.float32([[1, 0, 0], [0, 1, 1]])
                percept_stack2 = cv2.warpAffine(percept_stack2, M, (80, 80))
                action_selected2 = self.select_action(self.perception2, percept_stack2)
            # print(np.shape(percept_stack), np.shape(percept_stack2))
            # cv2.imshow('p1', percept_stack)
            # cv2.imshow('p2', percept_stack2)
            # cv2.waitKey(1)
            new_percept_stack, reward = self.act_and_perceive(action_selected, percept_stack, action_selected2)
            ...
            if not self.train and self.environment.game_over():
                ...
                print(('p2 won ' if won else 'p1 won ') +
...
def main(args):
    ...
    agent = DeepLearningAgent(npixels, npixels, nactions, nimages, args.reset, args.train, players=args.players, step= args.model)
    ...
    x_t = agent.perception1.preprocess_percepts(raw_percept, False)
    first_percept_stack = np.stack((x_t, x_t, x_t, x_t), axis = 2)
    log.info('first stack created successfully')
    model1_path = 'pretrained/network-5499999'
    model2_path = 'pretrained/network-5499999'
    agent.load_progress(model1_path, model2_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        dest='ngames', type=int, default=5,
    )
    ...
    parser.add_argument(
        ...
        dest='train', action='store_true', default=False)
    parser.add_argument(
        '-c', '--clear',
    ...
    parser.add_argument(
        ...
        help='clears the folders where the state of the agent is saves. '
             'NOTE: use this to retrain the agent from scratch',
        dest='model', type=int, default=-1)

    parser.add_argument(
        '-p', '--player',
        help='clears the folders where the state of the agent is saves. '
             'NOTE: use this to retrain the agent from scratch',
        dest='players', type=int, default=3)
