...
class LearningAgent(Agent):
    ...
    def run_step(self, debug=False):
        ...
        emergency_distance = safe_distance *10
        emergency_front_speed = 1.0
	emergency_front_speed = 100.0
	...
	if self._front_r and (self._front_r[1][0] <= emergency_distance):
        ...
        if self._state == AgentState.NAVIGATING:
 	    ...
 	    control = self._local_planner.run_step(debug=debug, target_speed=40)
	   ...
	   # print("%lf" % ego_speed)
	    ...
	    control = self._local_planner.run_step(debug=debug, target_speed=front_vel*3.6)
